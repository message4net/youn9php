<?php
//$return_arr[0][0]=INC_DBPDO_FILE_PATH;
...
$result_login_count=count($result);
if ($result_login && $result_login_count==1){
	...
	require BASE_DIR.$_SESSION['app_name'].'/'.DIR_MDL.OPS_MDL_MENU.POSTFIX_MDL;
}else{
	$returnarr['hide']=array('main_right','main_left');
	$returnarr['show']=array('main_login');
	$returnarr['content']['tips_login']='<span style="float:left;font-size:12px;color:green"><b><i>用户名或密码有误，请重新输入</b></i></span>';
}
