...
foreach ($arr_k as $val1){
	...
	$k_arr_post=$val1;
	if (!isset($arr_rwb_init[$k_arr_post])){
		$arr_rwb_init[$k_arr_post]=array();
	}
	...
	if ($arr_uc_del[$val1]){
		$str_ic_del='';
		foreach ($arr_rwb_del[$val1] as $val2){
			$str_rwb_del.=$val2.',';
		}
		$str_rwb_del=substr($str_rwb_del,0,strlen($str_rwb_del)-1);
		$arr_sql_rwb_del[$val1]='delete from role_wordbook where role_id='.$val.' and wordbook_id in ('.$str_rwb_del.')';
	}
}

if ($str_uc_add!=''){
	$str_uc_add=substr($str_uc_add,0,strlen($str_uc_add)-1);
	$sql_uc_add='insert into role_wordbook values '.$str_uc_add;
