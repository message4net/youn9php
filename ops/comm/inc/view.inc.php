...
//							$_sql_tmp=$val['sql_main'].$val['sql_suffix'].$rec_id.$val['sql_postfix'].$val['sql_main1'];
						...
						if ($_result_tmp_menu){
							...
							//	$cat_row='a';
							//}
						    $_return_html.='<tr id="'.$this->id_suffix_rb.$val['id'].'_'.substr($this->id_cat_da_b,0,strlen($this->id_cat_da_b)-1).'_'.$val['parent_id'].'"><td id="'.$this->id_suffix_da.$this->id_cat_da_b.$val['id'].'"><input type="checkbox" id="alckall'.$val['id'].'" value="'.$val['name'].'"/>'.$val['name'].'</td><td id="'.$this->id_suffix_da.$this->id_cat_da_c.$val['id'].'">';
